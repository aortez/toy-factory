...
static bool valid_request(const struct picosystem_garden_world *world,
			  const struct toy_factory_garden_gap *result)
{
	return (world != NULL) && (result != NULL) &&
	       (world->plant_count <= PICOSYSTEM_GARDEN_MAX_PLANTS) &&
	       (world->logic_tick_count % PICOSYSTEM_GARDEN_ECOLOGY_TICK_DIVISOR == 0U);
}

static bool eligible_adult(const struct picosystem_garden_plant *plant)
{
	return ((plant->flags & PICOSYSTEM_GARDEN_PLANT_DEAD) == 0U) &&
	       (plant->age_ecology_ticks >= PICOSYSTEM_GARDEN_SUN_CYCLE_TICKS);
}

static int export_adult(struct picosystem_garden_world *world,
			const struct picosystem_garden_plant *target, bool named,
				 ...
				 struct toy_factory_garden_gap *result)
{
	...
	return target == NULL ? -ENOENT : export_adult(world, target, false, result);
}

int toy_factory_garden_gap_apply_named(struct picosystem_garden_world *world, uint32_t lineage_id,
				       struct toy_factory_garden_gap *result)
{
	if (!valid_request(world, result) || (lineage_id == 0U)) {
		return -EINVAL;
	}
	for (uint8_t index = 0U; index < world->plant_count; ++index) {
		const struct picosystem_garden_plant *plant = &world->plants[index];
		if (plant->lineage_id == lineage_id) {
			return eligible_adult(plant) ? export_adult(world, plant, true, result)
						     : -EINVAL;
		}
	}
	return -ENOENT;
...
int toy_factory_garden_gap_print(const struct toy_factory_garden_gap *gap)
{
	...
	printf("{\"type\":\"gap\",\"protocol\":\"%s\",\"tick\":%" PRIu32 ",\"id\":%" PRIu32
	       ...
	       gap->named ? TOY_FACTORY_GARDEN_NAMED_GAP_PROTOCOL : TOY_FACTORY_GARDEN_GAP_PROTOCOL,
	       gap->tick, gap->lineage_id, gap->column, gap->nodes, gap->energy, gap->water,
	       gap->before_hash, gap->after_hash);
