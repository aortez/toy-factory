...
static int publish_snapshot(struct picosystem_game_demo_state *state, bool notify_renderer)
{
	...
	const int err = picosystem_game_snapshot_build(&state->world, sequence,
						       state->redraw_request_sequence,
						       k_uptime_ticks(), &snapshot);
	if (err != 0) {
		return err;
	}
...
int picosystem_game_demo_init(struct picosystem_game_demo_state *state)
{
	...
	err = picosystem_game_snapshot_build(&state->world, state->snapshot_sequence,
					     state->redraw_request_sequence, k_uptime_ticks(),
					     &initial_snapshot);
	if (err != 0) {
		return err;
	}
				   ...
				   struct picosystem_game_demo_stats *stats)
{
	...
	*stats = (struct picosystem_game_demo_stats){
	...
	if (state->world.scene_id == PICOSYSTEM_GAME_SCENE_GARDEN) {
		...
		stats->garden_climate_mode = garden->climate_mode;
		stats->garden_rain_rate = picosystem_garden_world_rain(garden);
		stats->garden_climate = picosystem_garden_world_climate(garden);
